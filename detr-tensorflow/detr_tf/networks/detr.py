...
class DETR(tf.keras.Model):
    def __init__(self, num_classes=92, num_queries=100,#500 to try for more people
                 backbone=None,
                 pos_encoder=None,
                 transformer=None,
    ...
    def call(self, inp, training=False, post_process=False):
        ...
        hs = self.transformer(self.input_proj(x), tf.cast(masks, tf.float32), self.query_embed(tf.constant(1.0)),
...
def add_heads_nlayers(config, detr, nb_class):
    ...
    # image_input = tf.keras.Input((480, 720, 3))#(1, 480, 720, 3)
    ...
    #outputs=[layer_output, x]
    n_detr = tf.keras.Model(image_input, outputs, name="detr_finetuning")
    #n_detr = tf.keras.Model(image_input, outputs=[outputs, attn], name="detr_finetuning")
    return n_detr

def get_detr_model(config, include_top=False, nb_class=None, weights=None, tf_backbone=False, num_decoder_layers=6, num_encoder_layers=6, maps=False):
    """ Get the DETR model
    ...
    #         att_maps = transformer(input_proj(x), masks, query_embed(None), pos_encoding)
    #         print(sess.run(att_maps[0]))
    #     return att_maps[0]

    #hs = transformer(input_proj(x), tf.cast(masks,tf.float32), query_embed(None), pos_encoding)[0]
    #print(input_proj(x).shape) #(None, None, None, 256)

    hs = transformer(input_proj(x), tf.cast(masks,tf.float32), query_embed(tf.constant(1.0)), pos_encoding)[0]

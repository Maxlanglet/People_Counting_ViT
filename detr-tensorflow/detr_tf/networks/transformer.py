...
class Transformer(tf.keras.Model):
    ...
    def call(self, source, mask, query_encoding, pos_encoding, training=False):
        ...
        #print("target", target.shape) #(100, None, 256)
        #print("IN query, pos and mask:",query_encoding.shape, pos_encoding.shape, mask.shape)
        memory = self.encoder(source, source_key_padding_mask=mask,
                              pos_encoding=pos_encoding, training=training)

        #print("memory :",memory.shape)
        hs = self.decoder(memory, target, memory_key_padding_mask=mask,
...
#     source : (345, 1, 256)
    ...
    #         input_shape = (None, None, self.model_dim)
...
class TransformerDecoder(tf.keras.Model):
    ...
    def call(self, memory, target, target_mask=None, memory_mask=None,
...
class DecoderLayer(tf.keras.layers.Layer):
    ...
    def call(self, memory, target, target_mask=None, memory_mask=None,
        ...
        # print("attn target :", attn_target.shape)
        ...
        # print("\nIN query tgt :",query_tgt.shape)
        # print("IN key mem :",key_mem.shape)
        # print("IN memory :",memory.shape)
        
        attn_target2 = self.multihead_attn((query_tgt, key_mem, memory), attn_mask=memory_mask,
                                           key_padding_mask=memory_key_padding_mask,
                                           need_weights=False)
        #print(attn_target2.shape)

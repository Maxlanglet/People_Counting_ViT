...
def np_tf_linear_sum_assignment(matrix):
    ...
    return [target_indices.astype(np.int64), pred_indices.astype(np.int64)
    , target_selector, pred_selector]
...
def hungarian_matching(t_bbox, t_class, p_bbox, p_class, fcost_class=1, fcost_bbox=5, fcost_giou=2, slice_preds=True) -> tuple:
    ...
    if slice_preds:
        size = tf.cast(t_bbox[0][0], tf.int64)
    ...
    # print(type(tf.cast(cost_matrix,tf.int64)))

    selectors = tf.numpy_function(np_tf_linear_sum_assignment, [tf.cast(cost_matrix,tf.int64)], [tf.int64, tf.int64, tf.bool, tf.bool] )
